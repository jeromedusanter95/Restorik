package com.jeromedusanter.restorik.core.ui.navigation
...
object LocalResultEventBus {
    ...
    infix fun provides(
        ...
        return LocalResultEventBus.provides(value = bus)
    }
}
...
class ResultEventBus {
    ...
    inline fun <reified T> removeResult(resultKey: String = T::class.toString()) {
        channelMap.remove(key = resultKey)
    }
}
...
inline fun <reified T> ResultEffect(
    ...
    LaunchedEffect(resultKey, resultEventBus.channelMap[resultKey]) {
        resultEventBus.getResultFlow<T>(resultKey = resultKey)?.collect { result ->
